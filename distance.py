    ...
    async def run_single(self, data: DataProto) -> dict:
        ...
        async with self._semaphore:
            try:
                ...
                if self._samples:
                    fen = decode_fen(response_str, "v0-verbose")
                    min_dist = min_fen_distance(fen, self._samples)
                    reward_extra_info["min_fen_distance"] = min_dist
                    if min_dist < fen_distance_threshold:
                        reward = 0
                        reward_extra_info['score'] = 0
                else:
                    reward_extra_info["min_fen_distance"] = 0.0

                if score == 1:
                    self._samples.append(expand_fen(decode_fen(response_str)))
